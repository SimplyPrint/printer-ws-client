...
from ..const import SNAPSHOT_ENDPOINT, VERSION
...
class Client:
    ...
    def __init__(self, *args, **kwargs):
        for _, value in Client.__dict__.items():
            if hasattr(value, "_type"):
                self.handles[value._type] = value
        
        self.connection = Connection(logging.getLogger("simplyprint.client.connection"))
        self.config = Config(*args)
        self.webcam = kwargs.get("webcam", Webcam())
    ...
    def start(self) -> None:
        ...
        # start AsyncLoop (if not already started)
    ...
    def __get_cpu(self) -> Dict[str, Any]:
        ...
        temperature_keys = {"coretemp", "cpu-thermal",
                            "cpu_thermal", "soc_thermal"} & set(temperatures.keys())
    ...
    def status(self, status: PrinterStatus) -> None:
        ...
        self.__logger.debug(
            f"Status changed from {self.printer.status} to {status}")
    ...
    def webcam_connected(self) -> bool:
        return self.webcam.connected

    @webcam_connected.setter
    def webcam_connected(self, connected: bool) -> None:
        if self.webcam.connected == connected:
            return

        self.webcam.connected = connected
        self.send_webcam_connected()
    ...
    async def send_temperatures(self) -> None:
        ...
        self.printer.server_bed_temperature = copy.deepcopy(
            self.printer.bed_temperature)
        self.printer.server_tool_temperatures = copy.deepcopy(
    ...
    def send_webcam_connected(self):
        self.__logger.debug(
            f"Sending webcam connected: {self.webcam.connected}")
    ...
    def get_base64_url(self, url: str) -> Optional[str]:
        ...
        try:
            response = requests.get(
                url, headers=headers, verify=False, timeout=2)
        except Exception:
            return None
    ...
    def set_display_message(self, message: str, short_branding: bool = False, spawn_loop: bool = True) -> None:
        ...
        self.__display_message_loop = self.loop.spawn(
            self.display_message_loop())
    ...
    def update_display_message(self, spawn_loop: bool = True) -> None:
        ...
        if self.status == PrinterStatus.PRINTING:
            self.set_display_message(
                f"Printing '{self.selected_file}'", spawn_loop=spawn_loop)
            return
    ...
    async def handle_new_token_event(self, event: NewTokenEvent) -> None:
        self.__logger.info(
            f"Received new token: {event.token} short id: {event.short_id}")
    ...
    async def handle_webcam_snapshot_event(self, event: WebcamSnapshotEvent) -> None:
        ...
        if not snapshot is None:
            ...
            self.__logger.error(
                f"Snapshot was requested but None was returned")
